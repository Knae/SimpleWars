...
sf::RenderTexture* CUIManager::m_pSceneBackground;
...
const CUnitEnums::TYPE CUIManager::m_UnitOnButton[]= { CUnitEnums::TYPE::INFANTRY, CUnitEnums::TYPE::TANK, CUnitEnums::TYPE::ARTILLERY };
...
bool CUIManager::IntializeUI(sf::Vector2u _inWindowSize, const unsigned int _inSceneWidth)
{
	...
	m_pSceneBackground->create(_inWindowSize.x- m_uSceneWidth, _inWindowSize.y);
	m_pSceneBackground->clear(sf::Color::Black);
...
bool CUIManager::ProcessClick(sf::Vector2f& _inCoords)
{
	...
	// Since the buttons are created in order,numbers 0 to 5 can represent them.
	//0=standard, 1=Start, 2=Dest, 3=Obstacle, 4=Start process
	//	switch (buttonClicked)
	//	{
	//		case 0:
	//		{
	//			m_eCurrentMouseState = MOUSESTATE::NEUTRAL;
	//			break;
	//		}
	//		case 1:
	//		{
	//			m_eCurrentMouseState = MOUSESTATE::START;
	//			break;
	//		}
	//		case 2:
	//		{
	//			m_eCurrentMouseState = MOUSESTATE::FINISH;
	//			break;
	//		}
	//		case 3:
	//		{
	//			m_eCurrentMouseState = MOUSESTATE::OBSTACLE;
	//			break;
	//		}
	//		case 4:
	//		{
...
///		Then we can be certain even if the images change, it'll be
///		what we expect.
/// </summary>
void CUIManager::SetUpUnitPlacementPanel()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		currentButton->setPosition( sf::Vector2f(m_uSceneWidth+16.0f,72.0f+(i*48)) );
